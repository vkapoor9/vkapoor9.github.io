...
metrics:
  ...
  - { label: "Daily briefing", value: "5:30 AM" }
...
The project scanning, daily pulses, Telegram briefing, and infra integrations all work. The piece I'm rebuilding is the task timeline: the nightly rescan currently rewrites task state on each run, so task history isn't reliable yet. The fix is to make the scanner additive instead of destructive, which is scoped and in progress. I'd rather say that plainly than badge it "Live."
